...
export async function sendSolicitud({ usuario, data, logo }) {
  ...
  const adjuntos = logo ? [{ filename: `logo-${data.empresa.razonSocial.replace(/[^\w-]+/g, '_')}${basename(logo.path).match(/\.\w+$/)?.[0] || ''}`, content: readFileSync(logo.path), contentType: logo.mimetype }] : [];
  await send({
